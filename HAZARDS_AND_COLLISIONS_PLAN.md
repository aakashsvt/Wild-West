...
- **Universal Jump Physics**: Ported the dynamic jump height physics bypass from Hurdles to the global `Hazard.tsx` wrapper. Now, *all* game obstacles (crates, terrain bumps, etc.) physically evaluate the horse's exact Y-height during a jump to determine if they are cleared or if they trigger a crash.
- **Walking Speed Resilience**: Added speed gating to hurdles. If the player walks or trots into a hurdle, the hurdle still shatters but the player forces their way through without suffering an involuntary stumble/loss of control.
- **Recoil Physics Tuning**: Significantly nerfed the massive backward slide that occurred upon major collisions while sprinting. Reduced the initial physics impulse from 25 to 8.0, and drastically cut the active-sprint continuous push multiplier from 0.4x down to 0.1x.
- **Impact Threshold Balancing**: Increased the minimum speed required to trigger a "Major Crash" (Ragdoll/Fall) from 45 to 48 for a more forgiving grace period while running.
- **Unconscious State Engine**: Updated `PlayerController.tsx` to precisely track the `wasStunnedLastFrame` state, dispatching a global `player-unconscious` event exactly when the player enters and recovers from the `FALL` state. (Used to trigger cinematic overlays).
